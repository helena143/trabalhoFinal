<template>
  <div class="ivy-sale-page">
    ...
    <transition name="modal-anim">
      <div v-if="selectedProduct && !checkoutOpen" class="modal-overlay" @click.self="closeModal">
        <div class="modal-box">
          ...
          <div class="modal-body">
            ...
            <div class="modal-image-side">
              <div
                class="modal-badge"
                :class="getMaterialType(selectedProduct) === 'dourado' ? 'dourado-badge' : 'prata-badge'"
              >
                {{ getMaterialType(selectedProduct) === 'dourado' ? 'Ouro 18k' : 'Prata 925' }}
              </div>
              ...
              >
                <img
                  ref="zoomImg"
                  :src="activeModalImage === 'secondary' ? selectedProduct.image2 : selectedProduct.image"
              ...
              <div class="modal-gallery-thumbs">
                <div
                  :class="['thumb-wrapper', { active: activeModalImage !== 'secondary' }]"
                  @click="activeModalImage = 'primary'; resetZoom()"
                >
                  <img :src="selectedProduct.image" :alt="selectedProduct.name + ' - imagem 1'" class="thumb" />
                </div>
                <div
                  :class="['thumb-wrapper', { active: activeModalImage === 'secondary' }]"
                  @click="activeModalImage = 'secondary'; resetZoom()"
                >
                  <img :src="selectedProduct.image2" :alt="selectedProduct.name + ' - imagem 2'" class="thumb" />
                </div>
              </div>
            </div>
            ...
            <div class="modal-details">
              ...
              <p class="modal-eyebrow">
                Coleção IVY · {{ selectedProduct.category }}
              </p>
              ...
              <div class="modal-specs">
                ...
                <div class="spec">
                  ...
                  <span
                    class="spec-value"
                    :class="getMaterialType(selectedProduct) === 'dourado' ? 'dourado-text' : 'prata-text'"
                  >
                    {{ getMaterialType(selectedProduct) === 'dourado' ? '✦ Peça Dourada' : '◈ Peça em Prata' }}
                  </span>
                </div>
              </div>

              <div class="modal-pricing">
                <span v-if="selectedProduct.oldPrice && selectedProduct.oldPrice > selectedProduct.price" class="modal-old-price">{{ format(selectedProduct.oldPrice) }}</span>
                <span class="modal-price">{{ format(selectedProduct.price) }}</span>
              </div>
              <p class="modal-installments">
                ou 10x de {{ format(selectedProduct.price / 10) }} sem juros
              </p>

              <div class="modal-actions">
                <button class="btn-carrinho" @click="addToCart(selectedProduct)">
                ...
                <button class="btn-comprar" @click="openCheckout">
                  Comprar Agora
                </button>
              </div>
    ...
    <transition name="modal-anim">
      <div v-if="checkoutOpen" class="modal-overlay" @click.self="closeCheckout">
        <div class="modal-box modal-checkout">
          ...
          <div class="modal-details">
            ...
            <p class="modal-description">
              Você está comprando: <strong>{{ selectedProduct?.name }}</strong>
            </p>
            <div class="modal-pricing">
              <span class="modal-price">{{ format(selectedProduct?.price) }}</span>
            </div>
            <div class="modal-specs">
              <div class="spec">
                ...
                <span class="spec-value">Prazo de 3 a 7 dias úteis</span>
              </div>
              <div class="spec">
                <span class="spec-label">Pagamento</span>
                <span class="spec-value">Pix, Cartão ou Boleto</span>
              </div>
            </div>
            <div class="modal-actions">
              <button class="btn-carrinho" @click="closeCheckout">Voltar</button>
              <button class="btn-comprar" @click="confirmOrder">Confirmar Pedido</button>
            </div>
          </div>
        </div>
      </div>
...
const activeModalImage = ref('primary')
...
const toggleMaterial = (mat) => {
  if (selectedMaterials.value.includes(mat)) {
    selectedMaterials.value =
      selectedMaterials.value.filter(m => m !== mat)
  } else {
...
const openModal = (product) => {
  ...
  activeModalImage.value = 'primary'
...
const products = ref([
  {
    ...
    price: 850,
    oldPrice: 1200,
    image: 'http://localhost:3000/public/products/chome.png',
   
    material: 'Prata 925 com Banho de Ródio',
    desc: 'O Colar Riviera é um ícone de sofisticação.'
  },

  {
    ...
    price: 700,
    oldPrice: 980,
    image: 'http://localhost:3000/public/products/colar5.png',
   
    material: 'Banho de Ouro 18k e Verniz Antialérgico',
    desc: 'A malha Veneziana é famosa por sua resistência.'
  },

  {
    id: 3,
    name: 'Relógio Luxo Black',
    category: 'Colares',
    price: 130,
    oldPrice: 200,
    image: 'http://localhost:3000/public/products/relogio6.png',
    material: 'Pérola de Água Doce Selecionada',
    desc: 'Nada supera o charme das pérolas.'
  },

 {
  ...
  desc: 'Inspirado nas grandes maisons.'
},

{
  ...
  price: 290,
  oldPrice: 380,
  image: 'http://localhost:3000/public/products/perfume3.png',
  material: 'Eau de Parfum Premium',
  desc: 'Perfume intenso e sofisticado.'
},

{
  ...
  price: 2500,
  oldPrice: 3890,
  image: 'http://localhost:3000/public/products/alianças.png',
  material: 'Ouro Amarelo 18k',
  desc: 'Elegância clássica e acabamento premium.'
},

{
  ...
  price: 580,
  oldPrice: 800,
  image: 'http://localhost:3000/public/products/berloque.png',
  material: 'Prata 925 e Zircônia Cristal',
  desc: 'Pulseira luxuosa com brilho intenso.'
},

{
  id: 9,
  name: 'Correntinha Cruz',
  category: 'Pingentes',
  price: 120,
  oldPrice: 190,
  image: 'http://localhost:3000/public/products/cruz.png',
  material: 'Prata Maciça 925',
  desc: 'Símbolo clássico com design moderno.'
},

{
  ...
  price: 87,
  oldPrice: 120,
  image: 'http://localhost:3000/public/products/berloque6.png',
  material: 'Prata 925',
  desc: 'Peça delicada e sofisticada.'
},

{
  id: 15,
  name: 'Aliança Solitário',
  category: 'Aliança',
  price: 320,
  oldPrice: 450,
  image: 'http://localhost:3000/public/products/brilho.png',
  material: 'Ouro Branco 18k',
  desc: 'Sofisticação e luxo em uma peça única.'
},

{
  ...
  price: 980,
  oldPrice: 1400,
  image: 'http://localhost:3000/public/products/dourada.png',
  material: 'Ouro 18k e Diamante',
  desc: 'Anel elegante com acabamento premium.'
},

{
  ...
  price: 220,
  oldPrice: 300,
  image: 'http://localhost:3000/public/products/perfume.png',
  material: 'Eau de Parfum 50ml',
  desc: 'Notas florais sofisticadas.'
},
...

{
  ...
  price: 540,
  oldPrice: 700,
  image: 'http://localhost:3000/public/products/relogio4.png',
  image2: 'http://localhost:3000/public/products/relogio4.png',
  material: 'Aço Inox',
  desc: 'Relógio sofisticado e elegante.'
},

{
  ...
  price: 390,
  oldPrice: 520,
  image: 'http://localhost:3000/public/products/relogio9.png',
  material: 'Silicone Premium',
  desc: 'Design esportivo e moderno.'
}
])
...
const filteredProducts = computed(() => {
  ...
  if (selectedMaterials.value.length) {
    list = list.filter(p => {
      const type = getMaterialType(p)

      return selectedMaterials.value.includes(type)
    })
  }
