    ...
    <div class="main-header">
      <div class="container-fluid header-grid">
        ...
        <div class="nav-group-right">
                    ...
                    <ul v-else>
                      ...
                      >
                        ...
                        <div class="res-info">
                          ...
                          <span class="res-cat">{{ p.category?.name || 'IVY' }}</span>
                          <span class="res-price">{{ formatarPreco(p.price) }}</span>
                        </div>
          ...
          <User class="nav-icon" @click="goLogin" :size="22" stroke-width="1.5" />

          <div class="bag-wrapper" @click="goCart">
            <ShoppingBag :size="22" stroke-width="1.5" />
            <span v-if="cart.items.length" class="bag-count">{{ cart.items.length }}</span>
          </div>
...
/* ══════════════════════════════════════════
   CATÁLOGO — carregado do banco de dados
   ══════════════════════════════════════════ */
const catalogoProdutos = ref([])
...
const carregarCatalogo = async () => {
  try {
    ...
    const lista = Array.isArray(data) ? data : (data.products || data.data || [])

    // Mapeia para garantir que os campos existam
    catalogoProdutos.value = lista.map(p => ({
      ...p,
      name:  p.name  || p.nome  || "",
      price: p.price || p.preco || 0,
      image: p.image || p.imagem || null,
      category: p.category || { name: "" }
    }))

    console.log(`[NavBar] ${catalogoProdutos.value.length} produtos carregados do banco`)
  } catch (err) {
    console.error("[NavBar] Erro ao carregar catálogo:", err?.response?.data || err.message)
...
const toggleSearch = () => {
  ...
  if (!searchActive.value) { searchText.value = ""; resultadosBusca.value = [] }
}
...
const buscarProdutos = () => {
  ...
  const termo = normalizar(searchText.value.trim())
  ...
  searchTimer = setTimeout(() => {
    resultadosBusca.value = catalogoProdutos.value.filter((p) =>
      normalizar(p.name).includes(termo) ||
      normalizar(p.category?.name || "").includes(termo)
    )
    buscando.value = false
  }, 200)
}

/* Mapa de categoria → rota */
const rotasPorCategoria = {
  "joias":      "/joias",
  "pingentes":  "/pingentes",
  "casamento":  "/casamento",
  "perfumes":   "/perfumes",
...
const irParaProduto = (produto) => {
  const cat   = normalizar(produto.category?.name || "")
  const rota  = rotasPorCategoria[cat] || "/joias"
  router.push(rota)
  fecharBusca()
}
...
.ivy-header { background: #fff; width: 100%; z-index: 1000; border-bottom: 1px solid #f2f2f2; }
...
.search-box { position: absolute; right: 0; width: 0; opacity: 0; overflow: hidden; transition: all 0.4s cubic-bezier(.22,1,.36,1); display: flex; align-items: center; }
.search-wrapper.is-active .search-box { width: 320px; opacity: 1; border-bottom: 1.5px solid #0a192f; overflow: visible; }
.search-box input { width: 100%; border: none; outline: none; padding: 6px 4px; font-family: 'Inter', sans-serif; font-size: 13px; background: transparent; color: #0a192f; }
...
.search-results { position: absolute; top: calc(100% + 18px); right: -10px; width: 380px; background: #fff; border: 1px solid #ececec; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,.13); overflow: hidden; z-index: 9999; }
.search-loading { display: flex; align-items: center; gap: 10px; padding: 20px; font-size: 13px; color: #999; }
.search-spinner { width: 16px; height: 16px; border: 2px solid #eee; border-top-color: #d4af37; border-radius: 50%; display: inline-block; animation: spin .6s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }
.search-empty { padding: 28px 20px; text-align: center; font-size: 13px; color: #999; display: flex; flex-direction: column; gap: 4px; }
.search-empty strong { color: #0a192f; font-size: 14px; }

ul { list-style: none; margin: 0; padding: 8px 0; }
li { display: flex; align-items: center; gap: 12px; padding: 10px 16px; cursor: pointer; transition: background .18s; border-bottom: 1px solid #f8f8f8; }
li:last-child { border-bottom: none; }
li:hover { background: #faf7f2; }
li:hover .res-arrow { opacity: 1; transform: translate(2px, -2px); }

.res-img { width: 44px; height: 44px; border-radius: 10px; background: #f5f5f5; overflow: hidden; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
...
.search-footer { padding: 10px 16px; background: #fafafa; border-top: 1px solid #f0f0f0; font-size: 11px; color: #bbb; letter-spacing: 0.5px; text-align: right; }
