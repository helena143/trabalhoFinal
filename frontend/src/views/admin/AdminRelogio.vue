<template>
  <AdminLayout>
    <div class="page-head">
      <div><h1>Relógios</h1><p>Gerencie o catálogo de relógios</p></div>
      <button class="add-btn" @click="openModal()">+ Novo(a) Relógio</button>
    </div>

    <div v-if="loading" class="loading">Carregando...</div>
    <div v-else-if="items.length === 0" class="empty">Nenhum(a) relógio cadastrado(a).</div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-head">
          <h2>{{ editing ? 'Editar Relógio' : 'Novo(a) Relógio' }}</h2>
...
const BASE = 'http://localhost:3000/api/relogios'
const items = ref<any[]>([])
