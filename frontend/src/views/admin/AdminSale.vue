<template>
  <AdminLayout>
    <div class="page-head">
      <div><h1>Promoções</h1><p>Gerencie as promoções da loja</p></div>
      <button class="add-btn" @click="openModal()">+ Nova Promoção</button>
    </div>

    <div v-if="loading" class="loading">Carregando...</div>
    <div v-else-if="items.length === 0" class="empty">Nenhuma promoção cadastrada.</div>
    <div v-else class="grid">
      <div class="card" v-for="item in items" :key="item._id">
        <img v-if="item.image" :src="imgUrl(item.image)" alt="" />
        <div class="no-img" v-else><i class="bi bi-tag"></i></div>
        <div class="card-body">
          <h3>{{ item.name }}</h3>
          <p class="price">R$ {{ Number(item.price).toFixed(2) }}</p>
          <p v-if="item.discount" class="discount">{{ item.discount }}% OFF</p>
          <div class="actions">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label>Desconto (%)</label><input v-model.number="form.discount" type="number" min="0" max="100" /></div>
        </div>
...
const BASE = 'http://localhost:3000/api/sales'
const items = ref<any[]>([])
...
const form = ref({ name: '', description: '', price: 0, discount: 0 })
...
const openModal = (item?: any) => {
  ...
  form.value = item ? { name: item.name, description: item.description || '', price: item.price, discount: item.discount || 0 } : { name: '', description: '', price: 0, discount: 0 }
...
.discount{color:#ff7675;font-size:.85rem;margin-bottom:14px;font-weight:600}
