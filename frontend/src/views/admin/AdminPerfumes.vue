<template>
  <AdminLayout>
    <div class="page-head">
      <div><h1>Perfumes</h1><p>Gerencie o catálogo de perfumes</p></div>
      <button class="add-btn" @click="openModal()">+ Novo(a) Perfume</button>
    </div>

    <div v-if="loading" class="loading">Carregando...</div>
    <div v-else-if="items.length === 0" class="empty">Nenhum(a) perfume cadastrado(a).</div>
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal">
        <div class="modal-head">
          <h2>{{ editing ? 'Editar Perfume' : 'Novo(a) Perfume' }}</h2>
...
const BASE = 'http://localhost:3000/api/perfumes'
const items = ref<any[]>([])
